...
RegisterNetEvent('gl-ambulance:useStretcher', function()
	...
	local stretcher = CreateObject(hash, PlayerPos.x, PlayerPos.y, PlayerPos.z-1, true, false)
...
RegisterNetEvent('gl-ambulance:pushstretcher', function()
    ...
    AttachEntityToEntity(closestObject, PlayerPed, GetPedBoneIndex(PlayerPed, 28422), 0.0, -0.9, -0.90, 196.0, 180.0,
...
RegisterNetEvent('gl-ambulance:putOnStretcher', function()
    ...
    if DoesEntityExist(closestObject) then
        ...
        AttachEntityToEntity(ped, closestObject, 0, 0, 0.0, 1.52, 180.0, 180.0, 180.0, 90.0, false, false, true, true, 2, true)
        LayOnStretcher()
    end
end)
...
RegisterNetEvent('gl-ambulance:interactBag', function()
    if PlayerJob.name == 'ambulance' or PlayerJob.name == 'police' then
        exports['qb-menu']:openMenu({{
            ...
            header = 'Pincet',
            txt = 'Om teken te verwijderen . . .  of kogels.',
            params = {
            ...
            header = 'Brandwondencrème',
            txt = 'Wie zijn *** verbrand, moet op de blaren zitten',
            params = {
                event = "gl-ambulance:getItem",
                args = {
                    item = 'burncream'
            ...
            header = 'EHBO kit',
            txt = 'Zet je terug in verticale toestand',
            params = {
                event = "gl-ambulance:getItem",
                args = {
                    item = 'firstaid'
                ...
                header = 'Brancard',
                txt = 'Om zwaargewonden te vervoeren',
                params = {
                    event = "gl-ambulance:getItem",
                    args = {
                        item = 'stretcher'
...
RegisterNetEvent('gl-ambulance:sendPulseBack', function(target)
    ...
    local minPulse = 60
    local maxPulse = 80
    local status = 'Normale Hartslag'
    QBCore.Functions.TriggerCallback('gl-ambulance:isPlayerDead', function(isdead)
        if isdead then
            minPulse = 0
            maxPulse = 0
            status = 'Geen Hartslag'
        else
            if isShot then
                minPulse = 120
                maxPulse = 200
                status = 'Bloedingen omwille van zichtbare schotwonden'
            end
            if isMeleed then
                minPulse = 90
                maxPulse = 115
                status = 'Heeft diepe snijwonden/blauwe plekken'
            end
            if isBurned then
                minPulse = 20
                maxPulse = 50
                status = 'Brandwonden over hun hele lichaam'
            end
        end
...
RegisterNetEvent('gl-ambulance:bodyBag', function()
    ...
    QBCore.Functions.LoadModel(hash)
	bodyBag = CreateObject(hash, pedCoords, false, false, false)
	PlaceObjectOnGroundProperly(bodyBag)
	--FreezeEntityPosition(bodyBag, true)
	...
	SetEntityCoords(ped, -1763.55, -263.63, 48.1, 0, 0, 0, false)
    SetEntityHeading(ped, 300)
	Wait(100)
