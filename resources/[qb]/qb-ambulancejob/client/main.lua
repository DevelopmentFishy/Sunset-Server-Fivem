...
CreateThread(function()
    while true do
        ...
        if isInHospitalBed and canLeaveBed then
            sleep = 0
            --exports['jg-textui']:DrawText(Lang:t('text.bed_out'))
            exports['jg-textui']:DrawText('[E] Om uit bed te komen..')
...
if Config.UseTarget then
    ...
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations['checking']) do
            ...
            checkingCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    if doctorCount >= Config.MinimalDoctors then
                        --exports['jg-textui']:DrawText(Lang:t('text.call_doc'), 'left')
                        exports['jg-textui']:DrawText("[E] Roep een doktor")
                        CheckInControls('checkin')
                    else
                        exports['jg-textui']:DrawText('[E] Aanmelden')
                        --exports['jg-textui']:DrawText(Lang:t('text.check_in'), 'left')
                        CheckInControls('checkin')
                    end
                else
                    listen = false
        ...
        --             exports['jg-textui']:DrawText('[E] Ga in bed liggen')
