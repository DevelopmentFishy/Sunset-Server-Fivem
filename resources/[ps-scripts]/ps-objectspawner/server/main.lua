...
RegisterNetEvent("ps-objectspawner:server:CreateNewObject", function(model, coords, objecttype, options, objectname)
    ...
    if hasperms then
        if model and coords then
            local data = MySQL.query.await("INSERT INTO ps_objects (model, coords, type, options, name) VALUES (?, ?, ?, ?, ?)", { model, json.encode(coords), objecttype, json.encode(options), objectname })
...
CreateThread(function()
    local results = MySQL.query.await('SELECT * FROM ps_objects', {})
...
RegisterNetEvent("ps-objectspawner:server:DeleteObject", function(objectid)
    ...
    if hasperms then
        if objectid > 0 then
            local data = MySQL.query.await('DELETE FROM ps_objects WHERE id = ?', {objectid})
...
local function CreateDataObject(mode, coords, type, options, objectname)
    MySQL.query.await("INSERT INTO ps_objects (model, coords, type, options, name) VALUES (?, ?, ?, ?, ?)", { model, json.encode(coords), type, json.encode(options), objectname })
end
