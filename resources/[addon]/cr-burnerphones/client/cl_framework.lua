...
function BCUtils.HasItem(item)
    ...
    elseif Config.Framework.Framework == "ESX" then
        ESX.TriggerServerCallback('cr-burnerphones:server:HasItem', function(HasItem)
            p:resolve(HasItem)
        end, item)
    else
...
function BCUtils.GeneratePhoneLoot(item)
    local p = promise.new()
    if Config.Framework.Framework == "QBCore" then
        QBCore.Functions.TriggerCallback('cr-burnerphones:server:GeneratePhoneLoot', function(loot)
            p:resolve(loot)
        end, item)
    elseif Config.Framework.Framework == "ESX" then
        ESX.TriggerServerCallback('cr-burnerphones:server:GeneratePhoneLoot', function(loot)
            p:resolve(loot)
        end, item)
    else
        print(Lcl('debug_standalonefunction'))
...
--- Stops playing an emote
function BCUtils.StopEmote()
