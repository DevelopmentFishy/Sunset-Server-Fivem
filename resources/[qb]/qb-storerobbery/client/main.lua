...
function LockpickDoorAnim(time)
    time = time / 1000
    loadAnimDict('anim@scripted@player@mission@tun_table_grab@cash@')
    TaskPlayAnim(PlayerPedId(), 'anim@scripted@player@mission@tun_table_grab@cash@', 'grab', 3.0, 3.0, -1, 16, 0, false, false, false)
    openingDoor = true
    CreateThread(function()
        while openingDoor do
            TaskPlayAnim(PlayerPedId(), 'anim@scripted@player@mission@tun_table_grab@cash@', 'grab', 3.0, 3.0, -1, 16, 0, 0, 0, 0)
            Wait(2000)
            time = time - 2
            TriggerServerEvent('qb-storerobbery:server:takeMoney', currentRegister, false)
            if time <= 0 then
                openingDoor = false
                StopAnimTask(PlayerPedId(), 'anim@scripted@player@mission@tun_table_grab@cash@', 'grab', 1.0)
            end
        end
        currentRegister = 0
    end)
