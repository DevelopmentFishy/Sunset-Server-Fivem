...
RegisterServerEvent('fishy-koperdief:server:stealedbox', function(objectCoords)
	boxStolen[objectCoords] = true
  giveStealedItemsToPlayer()
end)

QBCore.Functions.CreateCallback('fishy-koperdief:server:getbox', function(source, cb, objectCoords)
    local objectCoords = objectCoords
      cb(boxStolen[objectCoords])
  end)

  QBCore.Functions.CreateCallback('fishy-koperdief:server:GetCops', function(source, cb)
    local src = source
      local amount = 0
