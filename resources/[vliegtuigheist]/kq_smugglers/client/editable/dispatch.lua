function PoliceDispatch(eventCoords, early)
    local system = Config.dispatch.system
    local blipData = Config.dispatch.blip

    local desc = Config.dispatch.description
    if early then
        desc = Config.dispatch.earlyDescription
    end

    if system == 'default' then
        TriggerServerEvent('kq_smugglers:server:sendDispatch', eventCoords, early)
    ...
    elseif system == 'cd-dispatch' then
        ...
        TriggerServerEvent('cd_dispatch:AddNotification', {
            ...
            flash = 0,
            unique_id = 'kq_smugglers_' .. tostring(math.random(0000000, 9999999)),
            blip = {
                sprite = blipData.sprite,
                scale = blipData.scale,
...
RegisterNetEvent('kq_smugglers:client:sendDispatch')
AddEventHandler('kq_smugglers:client:sendDispatch', function(coords, early)
    ...
    SendDispatchMessage(desc, Config.dispatch.eventName)
