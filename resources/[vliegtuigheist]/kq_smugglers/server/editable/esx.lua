if Config.esxSettings.enabled then
    ...
    ESX.RegisterUsableItem(Config.items.radioEncoder, function(source)
        TriggerClientEvent('kq_smugglers:client:useRadioEncoder', source)
    end)
    
    ESX.RegisterUsableItem(Config.items.hint, function(source)
        TriggerClientEvent('kq_smugglers:client:useHint', source)
    end)
    ...
    function CanPlayerAfford(player, amount)
        ...
        if xPlayer.getAccount('black_money').money < amount and xPlayer.getAccount('money').money < amount then
            return false
        end
