...
local function IsTargetLaststand(playerId)
    local retval = false
    local hasReturned = false
    QBCore.Functions.TriggerCallback('police:server:isPlayerLaststand', function(result)
        retval = result
        hasReturned = true
    end, playerId)
    while not hasReturned do
...
local function HandCuffAnimation()
    ...
    if isHandcuffed == true then
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "Cuff", 0.2)-- sound
    else
...
RegisterNetEvent('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        local playerId = GetPlayerServerId(player)
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) or IsTargetLaststand(playerId) then
...
RegisterNetEvent('police:client:CuffPlayerSoft', function()-- begin
