...
RegisterNetEvent('qb-bankrobbery:client:LaptopPacific', function()-- Fleeca Laptop Animation
...
function OnHackPacificDone(success)
    if success then
        ...
        TriggerServerEvent('qb-bankrobbery:server:laptop3')
    end
end   
-- Functions
                    ...
                    QBCore.Functions.Progressbar("security_pass", Lang:t("general.validating_bankcard"), math.random(5000, 10000), false, true, {
                        ...
                        exports['ps-ui']:Maze(function(success)
                            if success then
                                ...
                                StopAnimTask(ped, "anim@gangops@facility@servers@", "hotwire", 1.0)
                                Config.DoorlockAction(1, true)
                                QBCore.Functions.Notify(Lang:t("error.cancel_message2"), "error")
                            end
...
RegisterNetEvent('qb-bankrobbery:client:UseRedLaptop', function()
    ...
    QBCore.Functions.TriggerCallback('qb-bankrobbery:server:isRobberyActive', function(isBusy)
        if not isBusy then
            if CurrentCops >= Config.MinimumPacificPolice then
                if not Config.BigBanks["pacific"]["isOpened"] then
                    local hasItem = Config.HasItem({"laptop_red"})
                    if hasItem then
...
CreateThread(function()
    ...
    electronickitZone:onPlayerInOut(function(inside)
        ...
        if inside and not Config.BigBanks["pacific"]["isOpened"] then
            Config.ShowRequiredItems({
                [1] = {name = QBCore.Shared.Items["laptop_red"]["name"], image = QBCore.Shared.Items["laptop_red"]["image"]},
            }, true)
        else
            Config.ShowRequiredItems({
                [1] = {name = QBCore.Shared.Items["laptop_red"]["name"], image = QBCore.Shared.Items["laptop_red"]["image"]},
