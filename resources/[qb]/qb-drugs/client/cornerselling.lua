                ...
                if IsEntityDead(stealingPed) then
                    ...
                    if not Config.UseTarget and #(pos - pedpos) < 1.5 then
                        if not textDrawn then
                            textDrawn = true
                            exports['jg-textui']:DrawText(Lang:t("info.pick_up_button"))
                        end
    ...
    if hasTarget then
        while pedDist < 1.5 and not IsPedDeadOrDying(ped) do
            ...
            if getRobbed <= Config.RobberyChance then
                ...
                local movetoCoords = {x = moveto.x + math.random(1000, 5000), y = moveto.y + math.random(1000, 5000), z = moveto.z, }
                ...
                if pedDist2 < 1.5 and cornerselling then
                    ...
                    elseif not Config.UseTarget then
                        if not textDrawn then
                            textDrawn = true
                            exports['jg-textui']:DrawText(Lang:t("info.drug_offer", {bags = bagAmount, drugLabel = currentOfferDrug.label, randomPrice = randomPrice}))
                        end
