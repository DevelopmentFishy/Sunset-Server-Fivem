...
CreateThread(function()
    while true do
        SafeCodes = {
            ...
            [22] = math.random(1000, 9999),
            [23] = math.random(1000, 9999),
        }
        Wait((1000 * 60) * 40)
    end
end)
...
RegisterNetEvent('qb-storerobbery:server:takeMoney', function(register, isDone)
    ...
    if isDone then
        local bags = math.random(700, 900)
        local info = {
            worth = math.random(cashA, cashB)
        }
        Player.Functions.AddItem('geldrol', bags, false, info)
...
RegisterNetEvent('qb-storerobbery:server:SafeReward', function(safe)
    ...
    if luck <= 10 then
        ...
        if luck == odd then
            Wait(500)
            Player.Functions.AddItem('snsblauwprint', 1)
