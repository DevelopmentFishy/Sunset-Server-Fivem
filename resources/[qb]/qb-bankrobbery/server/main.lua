        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                 ...
                 elseif Config.RewardTypes[itemType].type == "money" then
                    local info = {
                        worth = math.random(10000, 15000)
                    }
                    ply.Functions.AddItem('geldrol', math.random(10000, 15000), false, info)
                    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['geldrol'], "add")
                end
            else
                ply.Functions.AddItem('laptop_blue', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['laptop_blue'], "add")
            end
        else
            ply.Functions.AddItem('snsblauwprint', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['snsblauwprint'], "add")
        end
    elseif type == "paleto" then
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                 ...
                 elseif Config.RewardTypes[itemType].type == "money" then
                    local info = {
                        worth = math.random(23000, 55000)
                    }
                    ply.Functions.AddItem('geldrol', math.random(23000, 55000), false, info)
                    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['geldrol'], "add")
                 end
            else
                ply.Functions.AddItem('snsblauwprint', 1)
    ...
    elseif type == "pacific" then
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                ...
                elseif Config.RewardTypes[itemType].type == "money" then
                    local info = {
                        worth = math.random(123000, 155000)
                    }
                    ply.Functions.AddItem('geldrol', math.random(123000, 155000), false, info)
                    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['geldrol'], "add")
                end
            else
                local info = {
                ...
                ply.Functions.AddItem('bpcombatpistol', 1, false, info)
                TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['bpcombatpistol'], "add")
            end
        else
            local chance = math.random(1, 2)
            local odd = math.random(1, 2)
            if chance == odd then
                ply.Functions.AddItem('bptec9', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['bptec9'], "add")
            else
                ply.Functions.AddItem('bpdeagle', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['bpdeagle'], "add")
            end
        end
    end
end)
...
RegisterNetEvent('qb-bankrobbery:server:laptop2', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    Player.Functions.RemoveItem('laptop_blue', 1)
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["laptop_blue"], "remove")
end)

RegisterNetEvent('qb-bankrobbery:server:laptop3', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    Player.Functions.RemoveItem('laptop_red', 1)
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["laptop_red"], "remove")
end)
