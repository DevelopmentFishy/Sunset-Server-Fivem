...
function SetupDropTarget(phone, entity, reward, loc)
	if Config.Framework.Interaction.UseTarget then
		local options = {
			{ name = "searchDrop", type = "client", event = "cr-burnerphones:client:PickUpDrop", icon = "fa-solid fa-hand", label = Lcl('interact_searchbox'), phone = phone, box = entity, reward = reward, loc = loc },
		}
