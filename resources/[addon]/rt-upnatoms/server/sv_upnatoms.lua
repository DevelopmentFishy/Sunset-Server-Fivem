    ...
    if biller.PlayerData.job.name == 'upnatoms' then
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount < 201 then
                    if amount and amount > 0 then
                    billed.Functions.RemoveMoney('bank', amount)
                    TriggerClientEvent('QBCore:Notify', source, 'Je hebt de klant gefactureerd', 'success')
                    TriggerClientEvent('QBCore:Notify', billed.PlayerData.source, 'Er werd jou € '..amount..' aangerekend voor jouw order bij Up-n-Atoms.')
                    ...
                    exports['qb-management']:AddMoney('upnatoms', amount)
                        ...
                        TriggerClientEvent('QBCore:Notify', source, 'Bedrag moet een positief getal zijn.', 'error')
                    end
                else 
                    TriggerClientEvent('QBCore:Notify', source, 'Bedrag kan niet groter zijn dan 200 euro.', 'error')
                end    
            else
                TriggerClientEvent('QBCore:Notify', source, 'Je kan niet factureren aan jezelf.', 'error')
            end
        else
            TriggerClientEvent('QBCore:Notify', source, 'Burger is niet wakker', 'error')
        end
    end
end)