...
function OnPaletoHackDone(success)
    ...
    TriggerServerEvent('qb-bankrobbery:server:laptop2')
    end
end
    ...
    bankCardAZone:onPlayerInOut(function(inside)
        ...
        if inside and not Config.BigBanks["paleto"]["isOpened"] then
            Config.ShowRequiredItems({
                [1] = {name = QBCore.Shared.Items["laptop_blue"]["name"], image = QBCore.Shared.Items["laptop_blue"]["image"]}
            }, true)
        else
            Config.ShowRequiredItems({
                [1] = {name = QBCore.Shared.Items["laptop_blue"]["name"], image = QBCore.Shared.Items["laptop_blue"]["image"]}
