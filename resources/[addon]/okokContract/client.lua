...
AddEventHandler('okokContract:doRequest', function()
	...
	if closestPlayer ~= -1 and playerDistance <= 3.0 then
		...
		exports['okokRequests']:requestMenu(closestPlayerID, 10000, "<i class='fas fa-car'></i>&nbsp;BUY VEHICLE", "Wil je dit voertuig kopen?", "okokContract:OpenContractInfo", "client")
	end
end)
...
AddEventHandler('okokContract:GetVehicleInfo', function(source_playername, date, description, price, source)
	...
	if closestPlayer ~= -1 and playerDistance <= 3.0 then
		...
		if DoesEntityExist(vehicle) and (vehDistance <= 3) then
			...
			DeleteObject(prop)
			exports['okokNotify']:Alert("VEHICLE", "Je moet bij uw voertuig staan om dit te kunnen doen", 10000, 'error')
		end
	else
		ClearPedTasks(PlayerPedId())
		ClearPedSecondaryTask(PlayerPedId())
		DeleteObject(prop)
		exports['okokNotify']:Alert("VEHICLE", "Je moet dichter bij de persoon staan om dit te kunnen doen ", 10000, 'error')
	end
end)
		...
		if not isBlacklisted then
			QBCore.Functions.TriggerCallback("okokContract:checkIfOwnsVehicle", function(ownsVehicle)
				if ownsVehicle then
					...
					exports['okokNotify']:Alert("VEHICLE", "Je bent geen eigenaar van dit voertuig", 10000, 'error')
				end
			end, vehProps.plate)
		else
			exports['okokNotify']:Alert("VEHICLE", "Je kan dit voertuig niet verkopen!", 10000, 'error')
		end
	else
		exports['okokNotify']:Alert("VEHICLE", "Je moet dichter bij het voertuig staan om dit te kunnen doen", 10000, 'error')
	end
end)
