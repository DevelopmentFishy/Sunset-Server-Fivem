...
QBCore.Commands.Add("jail", Lang:t("commands.escort"), {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
QBCore.Commands.Add("paymechanic", Lang:t("commands.paytow"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        local playerId = tonumber(args[1])
        ...
        if OtherPlayer then
            if OtherPlayer.PlayerData.job.name == "mechanic" then
...
QBCore.Commands.Add('101', Lang:t("commands.police_report"), {{name='message', help= Lang:t("commands.message_sent")}}, false, function(source, args)
	local src = source
    local message
...
QBCore.Functions.CreateCallback('police:server:isPlayerLaststand', function(_, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    local src = source
    local playerPed = GetPlayerPed(src) -- code zelf
    local targetPed = GetPlayerPed(playerId) -- speler id
    local playerCoords = GetEntityCoords(playerPed) -- speler ped
    local targetCoords = GetEntityCoords(targetPed)-- de speler
...
--     if Item then
--         if Config.Inventory == 'qs-inventory' or Config.Inventory == 'ps-inventory' then
