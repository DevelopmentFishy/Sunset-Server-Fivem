...
local function PoliceCall()
    ...
    if math.random(1, 100) <= chance then
        if Config.politieMeldingen == 'default' then
            TriggerServerEvent('police:server:policeAlert', Lang:t("stealboxes.politie_notification"))
   
        elseif 'ps' then
            exports['ps-dispatch']:koperdiefstal()
        else 
            print('Melding systeem staat niet juist!')
        end
        QBCore.Functions.Notify(Lang:t("stealboxes.politie_notified"))
    end 
end
...
local function startStealingBox(entity)
    QBCore.Functions.Progressbar("stealingBox", Lang:t("stealboxes.stealing_animation_label"), Config.searchTime, false, true, {
        ...
        if DoesEntityExist(entity) then
            ...
            TriggerServerEvent("fishy-koperdief:server:stealedbox", objectCoords)
            ...
            options = {
                {
                    ...
                    action = function(entity)
                        ...
                        TriggerEvent('fishy-koperdief:client:steal', entity)
                    end,
...
RegisterNetEvent('fishy-koperdief:client:steal', function(entity)
    QBCore.Functions.TriggerCallback("fishy-koperdief:server:GetCops", function(cops)
        if cops >= Config.NodigPolitie then
            local pos = GetEntityCoords(entity)
            local objectCoords = pos.x .. pos.y .. pos.z 
            QBCore.Functions.TriggerCallback("fishy-koperdief:server:getbox", function(occupied)
                if occupied then  
                    RemoveBoxFromScene(entity)
