...
if Config.Framework.Framework == "QBCore" then
    ...
    QBCore.Functions.CreateCallback('cr-burnerphones:server:GeneratePhoneLoot', function(_, cb, phone)
        local loot = BSUtils.GenerateLoot(phone)
        cb(loot)
    end)
...
elseif Config.Framework.Framework == "ESX" then
    ...
    ESX.RegisterServerCallback('cr-burnerphones:server:GeneratePhoneLoot', function(_, cb, phone)
        local loot = BSUtils.GenerateLoot(phone)
        cb(loot)
    end)
...
--- Generates Loot w probility values
---@param phone string -- Phone Item being used
function BSUtils.GenerateLoot(phone)
    local tbl = Config.Burnerphones[phone].Rewards
    ...
    for _, item in pairs(tbl) do
        item.weight = math.ceil(item.prob / total_prob * 100)
