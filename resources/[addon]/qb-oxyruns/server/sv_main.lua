...
local GetLaunderAmount = function()
    ...
    local min = 800 + (cops * 50) -- 300 base + 100 per cop minimum
    local max = 1200 + (cops * 150) -- 600 base + 190 per cop minimum
...
RegisterNetEvent('qb-oxyruns:server:Reward', function()
    ...
    if Player then
        -- Cash
        local cash = math.random(250, 350)
        ...
        if item and launder <= Config.LaunderChance then
            ...
            TriggerClientEvent('QBCore:Notify', src, "Je hebt wat geld wit gewassen...", "success", 2500)
        end
...
QBCore.Functions.CreateCallback('qb-oxyruns:server:StartOxy', function(source, cb)
    ...
    if Player.PlayerData.money.cash >= Config.StartOxyPayment then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Je hebt niet genoeg geld om dit te starten..", "error",  3500)
        cb(false)
    end
end)
